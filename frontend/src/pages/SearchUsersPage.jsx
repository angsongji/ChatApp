      ...
      const callApi = async () => {
        ...
        setResultArray(
          result.data.filter((user) =>
            containsNormalized(user.fullName, valueSearch)
          )
        );
      };
      callApi();
      ...
      {resultArray == null ? (
        ...
        <>
          {resultArray.length == 0 ? (
            <div className="w-full h-full flex flex-col items-center justify-center text-sm md:text-base gap-1 flex-wrap">
              <div className="w-2/3 md:w-1/3 h-fit">
                <svg
              ...
              <div className="w-3/4 text-center">
                Sorry, we couldn't find any results for <b>{valueSearch}</b>
              </div>
            </div>
          ) : (
